...
function displayTitle(): void {
  console.log(
    chalk.redBright(figlet.textSync("Todo CLI", { horizontalLayout: "full" }))
  );
}

function addTodo(): void {
  const task: string = readlineSync.question(chalk.blue("Enter the task: "));
...
function displayTodos(): void {
  ...
  todos.forEach((todo) =>
    console.log(chalk.cyan(`ID: ${todo.id} - Task: ${todo.task}`))
  );
}
...
function main(): void {
  ...
  while (true) {
    showMenu();
    const choice: string = readlineSync
